...
# We will select the expression values of these 50 probes to form the input matrix for igraph
# selected probesets
selectedProbes = c(as.character(up.all.data.rma$probeset),as.character(down.all.data.rma$probeset))

# subset the expression values of these probes from the annotated expval matrix
expval_top = expval_final[rownames(expval_final) %in% selectedProbes,]
